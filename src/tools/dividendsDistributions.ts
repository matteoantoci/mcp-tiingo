...
const dividendsDistributionsInputSchemaShape = {
...
type Input = z.infer<z.ZodObject<typeof dividendsDistributionsInputSchemaShape>>;

export const dividendsDistributionsTool: TiingoToolDefinition = {
  name: 'get_dividend_distributions',
  description: 'Fetches historical dividend and distribution data for a given ticker.',
  inputSchemaShape: dividendsDistributionsInputSchemaShape,
  handler: async (input: Input): Promise<unknown> => {
    ...
    const url = new URL(`${API_BASE}/${ticker}/distributions`);
