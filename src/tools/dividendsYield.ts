...
const dividendsYieldInputSchemaShape = {
...
type Input = z.infer<z.ZodObject<typeof dividendsYieldInputSchemaShape>>;

export const dividendsYieldTool: TiingoToolDefinition = {
  name: 'get_dividend_yield',
  description: 'Fetches historical dividend yield data for a given ticker.',
  inputSchemaShape: dividendsYieldInputSchemaShape,
  handler: async (input: Input): Promise<unknown> => {
    ...
    const url = new URL(`${API_BASE}/${ticker}/distribution-yield`);
